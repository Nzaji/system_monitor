...
def get_smart_data():
    ...
    try:
        ...
        return []
#capture des logs système,  application et sésurité
def get_windows_logs(log_types=["System", "Application", "Security"], max_events=5):
    ...
    return logs
#insertion des metriques dans la tabe metrique
def insert_metrics(metrics):
    ...
    conn.commit()
    conn.close()
#insertion des donnée smart dans la table smart
def insert_smart_data(smart_list):
    ...
    conn.commit()
    conn.close()
#insertion de log dans la table log
...
#l'exportation en cv des donnée
def export_to_csv():
    conn = mysql.connector.connect(**DB_CONFIG)
...
#la boucle principale de collecte
def monitor_system():
    setup_database()
